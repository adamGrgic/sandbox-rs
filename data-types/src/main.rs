...
fn main() {
    ...
    // In rust strings are not quite the same as they are in other languages
    // for instance we have &str and String types
    // the key distinction is that &str is immutable, cannot be owned
    // String is mutable and owned

    let foo: String = "Hello world!".to_string();
    println!("{}", foo);


    let dog: &str = "I am a dog woof";
